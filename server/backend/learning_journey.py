...
@learning_journey.route("/edit", methods=['POST'])
def edit_learningjourney():

    front_end_json = request.get_json()
    print(front_end_json)

    lj_id = front_end_json["ljid"]
    role = front_end_json["role"]
    courses = front_end_json["courses"]
    position = front_end_json["position"]
    skills = front_end_json["skills"]

    # Delete all LJ entries with submitted LJID
    lj_to_delete = LearningJourney.query.filter_by(lj_id=lj_id).all()
    ...
    # Create new LJ entries for the params passed through
    for i in range (len(skills)):
        learningjourney = LearningJourney(lj_id, role["id"], position["position_id"], skills[i]["skill_id"], courses[i]["course_id"])
        print(learningjourney)
    ...
    return jsonify(
        {
            "message": "Learning Journey has been successfully edited."
        }
    ), 201
